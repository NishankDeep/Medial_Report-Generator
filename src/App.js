...
const downloadReport = (index) => {
    ...
    if (input) {
      html2canvas(input, { scale: 2 }).then((canvas) => {
        ...
        const addHeader = (pdf, pageNumber) => {
          ...
          // pdf.addImage(mImg, 'JPEG', 10, 5, 25, 25); 
          pdf.setFontSize(16);
      ...
      <div id="reportsContainer" className="mt-8">
        ...
        {reports.map((report, index) => (
          ...
          >
            ...
            <p>Main Test Name: {report.mainTestName}</p>
  ...
  {/* <div className="left">
    <div className="header flex justify-center items-center mb-4">
      <div className="flex items-center">
        <img src="MainLogo.jpeg" alt="Clinic Logo" className="h-12 w-auto" />
        <h1 className="text-5xl font-bold mt-2 ml-4">SPARSH LAB</h1>
      </div>
    </div>
    <div className="top-space mt-6">
      <p>Sahitya Samaj Chowk, Jail road</p>
      <p>Daltonganj, 822101</p>
      <p>sparshclinicdaltonganj@gmail.com</p>
    </div>
  </div>
  <div className="right flex flex-col">
  <p className="font-bold mb-2">sparsh clinic daltonganj</p>
  <img src="Microscope.png" alt="Clinic Logo" className="h-12 w-auto mr-2" />
  <div className="flex items-center">
    
    <p>PHARMACY, LAB, CLINIC</p>
  </div>
</div> */}
            ...
            <div className="grid grid-cols-2 gap-4 mb-4 ml-8 mr-8">
              <div>
                <p>
            ...
            <div className="border border-gray-300 p-2 text-center">{currentReport.mainTestName}</div>
    ...
    <tbody>
      {currentReport.tests.map((test, index) => (
        <tr key={index}>
          ...
          <td className={`border border-gray-300 p-2 ${isValueOutOfRange(test.result, test.bioRefInterval) ? 'bg-red-200 font-bold' : 'font-bold'}`}>
            {test.result}
          </td>
          <td className="border border-gray-300 p-2">{test.units}</td>
          <td className="border border-gray-300 p-2">{test.bioRefInterval}</td>
            ...
            {/* <footer className="mt-20 mb-4 border-t border-gray-300 pt-2 text-center">
              <p className="text-lg font-semibold">:- END OF REPORT :-</p>
              <p>SAHITYA SAMAJ CHOWK, JAIL ROAD, DALTONGANJ</p>
              <p>PHONE NO - 9470944040, 9470944422</p>
              <p className="text-red-500">WISHING YOU A GOOD LIFE AND BE HEALTHY</p>
            </footer> */}
          </div>
        </div>
      )}
